...
__device__ size_t CalculateStridedOffset(
  size_t gid,
  CudaVec shape,
  CudaVec strides,
  size_t offset) {
  ...
  for (int32_t d = shape.size - 1; d >= 0; d--) {
    size_t index_d = gid % shape.data[d];
    gid /= shape.data[d];
    strided_offset += strides.data[d] * index_d;
  }
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  if (gid < size) {
    size_t strided_offset = CalculateStridedOffset(gid, shape, strides, offset);
    out[gid] = a[strided_offset];
  }
  /// END SOLUTION
}
